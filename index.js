...
// Nano Banana Chat page route
app.get('/nanobanana', (req, res) => {
  res.sendFile(path.join(__dirname, 'public', 'nanobanana.html'));
});
...
app.post('/api/nanobanana/chat', authenticateUser, upload.array('referenceImages', 3), async (req, res) => {
  try {
    ...
    if (!prompt || prompt.trim().length === 0) {
      return res.status(400).json({
        success: false,
        error: 'لطفاً یک توضیح برای تصویر وارد کنید'
      });
    }

    // Check and deduct credits (1 credit per Nano Banana generation)
    const creditCheck = await checkAndDeductCredits(userId, 'nanobanana');
    if (!creditCheck.allowed) {
      return res.status(403).json({
        success: false,
        error: creditCheck.message,
    ...
    console.log(`✅ Credits deducted. Remaining: ${creditCheck.remaining}`);
    ...
    if (!nanoBananaResult.imageData) {
      throw new Error('No image generated by Nano Banana');
    }

    // Convert base64 to buffer and upload to Supabase Storage
    const imageBuffer = Buffer.from(nanoBananaResult.imageData, 'base64');
    const fileName = `nanobanana-${userId}-${Date.now()}.png`;
    ...
    if (uploadError) {
      console.error('Error uploading Nano Banana image:', uploadError);
      throw uploadError;
    }
      ...
      .insert([
        {
          ...
          background_id: null,
          prompt: prompt,
          description: `Nano Banana Chat - ${aspectRatio}`,
          generated_image_url: generatedImageUrl,
          content_type: 'nanobanana',
          created_at: new Date().toISOString()
        }
      ])
      .select();
    ...
    if (dbError) {
      console.error('Error saving Nano Banana generation to DB:', dbError);
    }

    console.log('✅ Nano Banana image generated and saved successfully!');
